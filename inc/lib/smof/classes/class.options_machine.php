		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'heading':
					...
					//为了汉化中文在functions.options.php 特意增加CNNAME
          $value['cnname'] = $value['cnname'] ? $value['cnname']:$value['name'];
					$menu .= '<li class="'. $header_class .'"><a title="'.  $value['cnname'] .'" href="'.  (isset($value['redirect']) ? $value['redirect'] : ('#'.$jquery_click_hook))  .'"'. $icon .'><i class="header-icon'.(isset($value['icon']) ? " {$value['icon']}" : '').'"></i><span>'. $value['cnname'] .'</span></a></li>';
